...
namespace aes_impl
{
    ...
    {
        uint64_t val = 0;
        while (!_rdrand64_step(&val)) _mm_pause();
        return val;
    }
    ...
    static inline void secure_zero(void* p, size_t n) noexcept
    {
...
static inline bool encrypt_file(
...
static inline bool decrypt_file(
    const std::wstring& ipath,
    const std::wstring& opath,
    const std::array<uint8_t, 32>& key
) noexcept
